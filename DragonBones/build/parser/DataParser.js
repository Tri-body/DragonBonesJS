...
var BaseTimelineState_1 = require("../animation/BaseTimelineState");
var FrameData_1 = require("../model/FrameData");
var BaseObject_1 = require("../core/BaseObject");
var ObjectDataParser_1 = require("./ObjectDataParser");
...
var DataParser = /** @class */ (function () {
    function DataParser() {
        ...
        this._timelinePivot = new Point_1.Point(); // For 2.x ~ 3.x
        this._helpPoint = new Point_1.Point();
        this._helpTransformA = new Transform_1.Transform();
        this._helpTransformB = new Transform_1.Transform();
        this._helpMatrix = new Matrix_1.Matrix();
    ...
    DataParser.prototype._getTimelineFrameMatrix = function (animation, timeline, position, transform) {
        ...
        if (timeline.frames.length === 1 || frameIndex >= timeline.frames.length) {
            ...
            if (frame.tweenEasing !== DragonBones_1.DragonBones.NO_TWEEN) {
                tweenProgress = (position - frame.position) / frame.duration;
                if (frame.tweenEasing !== 0.0) {
                    tweenProgress = BaseTimelineState_1.TweenTimelineState._getEasingValue(tweenProgress, frame.tweenEasing);
                }
            }
            else if (frame.curve) {
                tweenProgress = (position - frame.position) / frame.duration;
                tweenProgress = BaseTimelineState_1.TweenTimelineState._getEasingCurveValue(tweenProgress, frame.curve);
            }
            ...
            transform.skewX = Transform_1.Transform.normalizeRadian(nextFrame.transform.skewX - frame.transform.skewX);
            transform.skewY = Transform_1.Transform.normalizeRadian(nextFrame.transform.skewY - frame.transform.skewY);
    ...
    DataParser.prototype._mergeFrameToAnimationTimeline = function (framePostion, actions, events) {
        ...
        if (frames.length === 0) {
            var startFrame = BaseObject_1.BaseObject.borrowObject(FrameData_1.AnimationFrameData); // Add start frame.
            ...
            if (this._animation.frameCount > 1) {
                ...
                var endFrame = BaseObject_1.BaseObject.borrowObject(FrameData_1.AnimationFrameData); // Add end frame to keep animation timeline has two different frames atleast.
    ...
    DataParser.parseDragonBonesData = function (rawData) {
        return ObjectDataParser_1.ObjectDataParser.getInstance().parseDragonBonesData(rawData);
    };
    /**
     * @deprecated
    ...
    DataParser.parseTextureAtlasData = function (rawData, scale) {
        ...
        for (var i = 0, len = subTextureList.length; i < len; i++) {
            ...
            var subTextureRegion = new Rectangle_1.Rectangle();
            ...
            if (DataParser.FRAME_WIDTH in subTextureObject) {
                subTextureFrame = new Rectangle_1.Rectangle();
